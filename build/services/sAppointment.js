...
const AppointmentDetail_1 = __importDefault(
  require("../models/AppointmentDetail")
);
const Examination_1 = __importDefault(require("../models/Examination"));
const constants_file_1 = __importDefault(
  require("../config/properties/constants.file")
);
let colorStatus = {
  S: {
    primary: "#E3BC08",
...
class AppointmentServicios {
  ...
  static getAppointmentAll(offset, limit, query) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const { count: total, rows } =
        yield Appointment_1.default.findAndCountAll({
          where,
      ...
      let datos = [];
      for (const r of rows) {
        const daton = (yield config_database_1.default.query(
          `SELECT 
            ...
            conve.id conveid, conve."name" convename, tpe.name nametypemploye

            FROM public."Appointments" a
            left join "Clients" c on c.id = a."ClientId"
			left join "Users" u on u.id = c."UserId"
            left join "Employees" emplo on emplo.id = a."ResponsibleId"
            left join "TypeEmployees" tpe on tpe.id = emplo."TypeEmployeeId"
        ...
        const daton3 = yield config_database_1.default.query(
          `SELECT distinct
        ex."name" listaexamen,
        exep.price listaprecios,
        ser.name nameservice
        ...
        inner join public."Services" as ser on (ser.id = ex."ServiceId")
        ...
        where ad."AppointmentId"=${r.get("id")}`,
          { type: sequelize_1.QueryTypes.SELECT }
        );
        const daton4 = (yield config_database_1.default.query(
          `SELECT distinct
        ex."name" nameexa,
        ser.name nameservice
        ...
        datos.push(
          Object.assign(Object.assign({}, persona), {
            colorStatus: Object.assign({}, colorStatus[persona.status]),
            client: {
              id: daton.clientid,
              ...
              genderStr:
                constants_file_1.default.GENDER_STR[daton.clientgender],
  ...
  static getAppointment(id) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      //console.log(cita);
      let datos = Object.assign(Object.assign({}, cita), {
        colorStatus: Object.assign({}, colorStatus[cita.status]),
  ...
  static getAppointmentsByDates(query) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (query.start && query.end) {
        ...
        where = {
          dateAppointment: { [sequelize_1.Op.between]: [start, end] },
          status: query.status,
        };
      }
      const { count: total, rows } =
  ...
  static getExamValues(appointmentId) {
    ...
    LEFT JOIN public."Services" as ser on (ser.id = e."ServiceId")
  ...
  static getExaminationGroupsID(examinationid) {
    return __awaiter(this, void 0, void 0, function* () {
      const examinatioGroups = yield config_database_1.default.query(
        `SELECT  distinct
        *
    FROM public."ExaminationGroups" e
    where e.status='A' and e."ExaminationId" = ${examinationid} `,
        { type: sequelize_1.QueryTypes.SELECT }
      );
  ...
  static getExaminationValueByExaminationGroupId(examinationgroupId) {
    return __awaiter(this, void 0, void 0, function* () {
      const examinationValue = yield config_database_1.default.query(
        `SELECT  distinct
        e.*,
        u.id idunit,
        u."name" nameunit,
    ...
    where e."ExaminationGroupId"=${examinationgroupId} `,
        { type: sequelize_1.QueryTypes.SELECT }
      );
  ...
  static getAppointmentComplete(id) {
    return __awaiter(this, void 0, void 0, function* () {
      const info = yield config_database_1.default.query(
        `SELECT  
		a.result,
		a."ResponsibleId",
  ...
  static getAppointmentResults(AppointmentId) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
      const services = yield config_database_1.default.query(
        `SELECT  distinct
        s.id as id,
        s."name" as name,
    ...
    where e.status ='A' and ap."AppointmentId"= ${AppointmentId} `,
        { type: sequelize_1.QueryTypes.SELECT }
      );
      const info = (yield config_database_1.default.query(
        `SELECT  
		...
		e."name" ||' ' || e."lastNameP" || ' ' || e."lastNameM" as responsible,
		a."SpecialityId" speid,
		s."name" spename,
		e."digitalSignatureUrl" edigi

	FROM public."Appointments" a
	left join public."Employees" e on e.id = a."ResponsibleId"
	left join public."Specialities" s on s.id = a."SpecialityId"
	where a.id= ${AppointmentId} `,
        { type: sequelize_1.QueryTypes.SELECT }
      ))[0];
      const info2 = (yield config_database_1.default.query(
        `SELECT  
    ad.result result
    ...
    where ad.id= ${AppointmentId} `,
      ...
      let data = {
        result: info2 ? info2.result : null,
      ...
      for (const service of services) {
        ...
        service.result = [];
        const examination = yield this.getExaminationByID(
          service.examinationid
        );
        examination.appointmentDetailId = service.apointmentid;
        const examinatioGroups = yield this.getExaminationGroupsID(
          service.examinationid
        );
        ...
        for (const group of examinatioGroups) {
          ...
          const examinationValue =
            yield this.getExaminationValueByExaminationGroupId(group.id);
          for (const value of examinationValue) {
            ...
            let result = {
              ...
              result: value.result,
              refValue:
                (_a = value.refvalue) !== null && _a !== void 0 ? _a : "",
            };
  ...
  static updateAppointment(id, data) {
    return __awaiter(this, void 0, void 0, function* () {
      config_database_1.default.transaction((transaction) =>
    ...
    return __awaiter(this, void 0, void 0, function* () {
      for (let ex of data.examinations) {
        for (let rV of ex.detalleExam) {
          yield config_database_1.default.query(
            `UPDATE public."ExaminationValues"
              SET "countVR2"='${rV.valorObtenido}', "updatedAt"=NOW()
              WHERE id = ${rV.id};`,
            { type: sequelize_1.QueryTypes.UPDATE }
          );
          
          let resultValue = ex.result ? `'${ex.result}'` : "''";
          yield config_database_1.default.query(
            `UPDATE public."AppointmentDetails"
              SET result=${resultValue}
              WHERE "AppointmentId"= ${AppointmentId} AND "ExaminationId" = ${ex.idexam};`,
            { type: sequelize_1.QueryTypes.UPDATE }
          );
  
          const apQuery = yield config_database_1.default.query(
            `
             SELECT distinct ap.id as apointmentid
             FROM public."Services" s 
             left join public."Examinations" e on s.id = e."ServiceId" 
             left join public."AppointmentDetails" ap on ap."ExaminationId" = e.id
             where e.status ='A' and ap."AppointmentId" = ${AppointmentId}`,
          ...
          const apointmentid = apQuery[0].apointmentid;
          console.log(apointmentid)
  
          try {
            const result = yield config_database_1.default.query(`
              INSERT INTO public."ExaminationDetails" 
              (value, status, "createdAt", "updatedAt", "AppointmentDetailId", "ExaminationValueId") 
              VALUES 
              ('prueba', 'A', NOW(), NOW(), '${apointmentid}', '${rV.id}')
              ON CONFLICT ("AppointmentDetailId", "ExaminationValueId")
              DO UPDATE SET 
      ...
      console.log(data.hasOwnProperty("ResponsibleId"))
