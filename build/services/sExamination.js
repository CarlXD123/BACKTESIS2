...
class ExaminationServicios {
    ...
    static getPagedExamination(offset, limit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            me.name nombremethod
            FROM public."Examinations" e
            inner join public."Services" s on s.id= e."ServiceId"
            inner join public."ExaminationGroups" eg on eg."ExaminationId" = e.id
            inner join public."ExaminationValues" ev on ev."ExaminationGroupId" = eg.id
            inner join public."Methods" me on me.id= ev."MethodId"
            where e.id = ${r.get('id')}`, { type: sequelize_1.QueryTypes.SELECT }))[0];
                if (!daton) {
                    console.log(`No se encontraron resultados para id: ${r.get('id')}`);
                    continue;
                }
    ...
    static getFilterExamination(query) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            e.name AS nombreexammen,
            e.id AS idxamem,
            me.name AS nombremethod
            FROM public."Examinations" e
            inner join public."Services" s on s.id= e."ServiceId"
            inner join public."ExaminationGroups" eg on eg."ExaminationId" = e.id
            inner join public."ExaminationValues" ev on ev."ExaminationGroupId" = eg.id
            inner join public."Methods" me on me.id= ev."MethodId"
            where e.id = ${r.get('id')} AND s.id= e."ServiceId"`, { type: sequelize_1.QueryTypes.SELECT }))[0];
                if (daton) {
                    const groups = yield this.getExaminationGroupsID(r.get('id'));
                    let persona = r.get();
                    let data = Object.assign(Object.assign({}, persona), { service: {
                    ...
                    console.error(`No data found for examination ID: ${r.get('id')}`);
                }
            }
            const result = {
                total,
