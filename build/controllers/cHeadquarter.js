...
class HeadQuarterController {
    ...
    getHeadquarter(_req, _res) {
        try {
            ...
            let headquarter = sHeadquarter_1.default.getHeadquarter(id);
            headquarter.then((h) => __awaiter(this, void 0, void 0, function* () {
                let result = {
                    "status": true,
                    "data": h
                };
                result.data = [];
                const agre = yield sHeadquarter_1.default.getHeadquarterByAgreement(id);
                let mapeado = {};
                mapper.map(h, mapeado);
                if (agre != undefined)
