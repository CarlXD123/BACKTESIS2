...
class EmployeeController {
    ...
    getEmployeeByUserId(_req, _res){
        try {
            const { userId } = _req.params;
            if (!userId) {
                // Devuelve una respuesta significativa si `userId` no está definido.
                return _res.status(400).json({ status: false, message: "UserId is required" });
            }
            
            let employees = sEmployee_1.default.getEmployeeByUserId(userId);
            employees.then((e) => {
                let result = {
                    "status": true,
                    "data": e
