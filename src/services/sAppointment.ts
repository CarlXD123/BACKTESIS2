...
class AppointmentServicios {
    ...
    public static async getAppointmentAll(offset: any, limit: any, query: any) {
        ...
        const daton4 = (await db.query<any>(`SELECT distinct
        ex."name" nameexa,
        ser.name nameservice
        ...
        inner join public."Agreements" as ag on (ag."id" = pl."AgreementId")
        inner join public."Clients" as cl on (cl."id" = ap."ClientId")
        where ad."AppointmentId"=${r.get('id')}`, { type: QueryTypes.SELECT }))[0];
    ...
    const info2 = (await db.query<any>(`SELECT  
    ad.result result

    FROM public."AppointmentDetails" ad
    left join public."Appointments" a on a.id = ad."AppointmentId"
    where ad.id= ${AppointmentId} `, { type: QueryTypes.SELECT }))[0];
    ...
    public static async attendAppointment(AppointmentId: any, data: any) {
        ...
        for (let ex of data.examinations) {
            ...
            for (let rV of ex.detalleExam) {
                ...
                await db.query(`UPDATE public."AppointmentDetails"
                SET result=${resultValue}
                WHERE "AppointmentId"= ${AppointmentId} AND "ExaminationId" = ${ex.idexam};`, { type: QueryTypes.UPDATE });

                 // Obtener el apointmentid de la consulta proporcionada
                 const apQuery = await db.query(`
                 ...
                 const apointmentid = (apQuery[0] as any).apointmentid; //asumir que el apointmentid que necesitas es el primero retornado por la consulta
