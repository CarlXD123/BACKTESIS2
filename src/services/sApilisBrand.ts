    ...
    public static async destroyBrand(id: any) {
        ...
        try {
            ...
            await ApilisMatchData.update(
                ...
                {
                    where: { idModel: { [Op.in]: modelIds }  }
                }
            );

            const examToUpdate = await ApilisMatchData.findAll({
                where: { 
                    idModel: { [Op.in]: modelIds },
                    codBaja: 1
                },
            ...
            const matchDetails = examToUpdate.map((matchdetail: any) => matchdetail.id);
            ...
            await ApilisMatchDataDetail.update(
                ...
                {
                    where: { idApilisMatchData: { [Op.in]: matchDetails}}
                }
            );
